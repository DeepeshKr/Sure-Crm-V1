class DistributorMissedPincode < ActiveRecord::Base
  ...
  def self.create_or_update_pincode(pincode, order_value)
    ...
    if pin_codes.present?
      ...
      total_orders = pin_code.no_of_orders + 1
      total_value = pin_code.total_value + order_value
      pin_code.update(no_of_orders: total_orders, total_value: total_value, last_ran_on: t)
    else
      DistributorMissedPincode.create(pincode: pincode,no_of_orders: 1, total_value: order_value, last_ran_on: t, description: "created the first record for this pincode on #{t}")
    end
  end
end
