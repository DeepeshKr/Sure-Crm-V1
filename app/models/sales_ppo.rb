class SalesPpo < ActiveRecord::Base
  ...
  def sales_ppos_between_dates from_date, to_date
    out_sales_ppos = []
    nos = 1
    ...
    sales_ppo_1 = daily_campaign_ppo day, true, false, 40.00, 65.00, "all"
    ...
    ret_sales_ppo.total_name_1 = "R 40% | T 65%"
    ret_sales_ppo.total_name_2 = "R 40% | T 65%"
    ...
    ret_sales_ppo.description = "R=> 40: T=>65"
  ...
  def sales_show_ppos_for_date from_date, to_date, product_variant_id
    out_sales_ppos = []
    nos = 1
  ...
  def sales_product_ppos_for_date from_date, to_date, product_variant_id
    out_sales_ppos = []
    nos = 1
   ...
   campaign_playlists.each do |playlist|
    ...
    sales_ppo_2 = calculate_product_ppo playlist.id, true, false, 40.00, 65.00, "all", product_variant_id
    #    byebug
    #retail_rate = final_return_rate.round(1) if final_return_rate.present?
    ret_sales_ppo = SalesPpo.new
    ret_sales_ppo.campaign_playlist_id = playlist.id
  ...
  def sales_hour_ppos_for_date from_date, to_date, start_total_secs, end_total_secs
    out_sales_ppos = []
    nos = 1
  ...
  def operator_ppos_for_date from_date, to_date, media_id
    out_sales_ppos = []
    nos = 1
   ...
   campaign_playlists.each do |playlist|
    ...
    sales_ppo_2 = operator_ppo playlist.id, true, false, 40.00, 65.00, "all", media_id
    #    byebug
    #retail_rate = final_return_rate.round(1) if final_return_rate.present?
    ret_sales_ppo = SalesPpo.new
    ret_sales_ppo.campaign_playlist_id = playlist.id
  ...
  def calculate_campaign_ppo campaign_playlist_id, show_all, show_shipped, ret_correct, to_correct, results
    ...
    tranfer_order_ids = [10020, 10040, 10041]
     ...
     sales_ppos = SalesPpo.where('order_status_id > 10002')
          ...
          sales_ppo.product_cost = ProductCostMaster.find_by_prod(campaign_playlist.product_variant.extproductcode).cost
          ...
          if retail_sales_ppos.present?
           ...
           if results=="all"
             ...
             sales_ppo.profit_per_order = ((sales_ppo.total_profit || 0) / (sales_ppo.cor_total_nos || 1)).round(2) 
