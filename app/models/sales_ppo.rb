class SalesPpo < ActiveRecord::Base
 ...
 #handle_asynchronously :update_all_ppos
  def change_ppo_product_costs order_id
  ...
  def add_product_to_campaign
   ...
   if campaign_playlist_id.blank?
     ...
     if extended_older_campaign_playlist.present? && campaign_playlist_id.blank?
        puts "Found campaign playlist #{campaign_playlist_id} in Extended Older Playlist and update this for order #{order_id}".colorize(:white)
  ...
  def self.to_new_csv
    ...
    CSV.generate(headers: true) do |csv|
       csv << ["Id","Show","Date Time","Product Cost"]
       all.each do |c|
         csv << [c.campaignid, c.show, c.for_date, c.product_cost]
       end
    end
  end
  ...
  def self.to_csv
    byebug
    CSV.generate(headers: true) do |csv|
      csv << ['Id','Show','Date Time','Product Cost','Description','Name R1','Nos R1','Pieces R1','Sales R1','Revenue R1','Product Cost R1','Var Cost R1','Fix Cost R1','Refund R1','Damages R1','PPO R1','Name R2','Nos R2','Pieces R2','Sales R2','Revenue R2','Product Cost R2','Var Cost R2','Fix Cost R2','Refund R2','Damages R2','PPO R2'] ## Header values of CSV

      all.each do |c|
      csv <<  [c.campaign_playlist_id, c.show, c.for_date, c.show_time, c.product_cost, c.description, c.total_name_1, c.total_nos_1, c.total_pieces_1, c.total_sales_1, c.total_revenue_1, c.total_product_cost_1, c.total_var_cost_1, c.total_fixed_cost_1, c.total_refund_1, c.total_product_dam_cost_1, c.profit_per_order_1, c.total_name_2, c.total_nos_2, c.total_pieces_2, c.total_sales_2, c.total_revenue_2, c.total_product_cost_2, c.total_var_cost_2, c.total_fixed_cost_2, c.total_refund_2, c.total_product_dam_cost_2, c.profit_per_order_2]
        ##Row values of CSV
      end
    end
  end
  ...
  def daily_campaign_ppo for_date, show_all, show_shipped, ret_correct, to_correct, results
    ...
    total_fixed_cost = campaign_playlists.first.group_total_cost.to_f if campaign_playlists.first.group_total_cost.present?
  ...
  def add_update order_id
    ...
    #add or update ppo details
      ...
      :shipping_cost => ordln.product_postage)
       ...
       response = "Updated existing Sales PPO with id #{@sale_ppo.id} created on #{@sale_ppo.created_at.strftime("%d-%b-%y %H:%M")} for order id #{order_master.id}"
      else
       ...
       response = "Create new Sales PPO with id #{@sale_ppo.id} for order id #{order_master.id}"
       end
     end
  end
