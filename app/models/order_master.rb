class OrderMaster < ActiveRecord::Base
  ...
  def update_customer_order_list(order_id)
    return nil if order_id.blank?
      ...
      return nil if pro_order_master.blank?
      ...
      if pro_order_master.orderpaymentmode_id == 10080
        creditcardno =  "4111111111111111"
        expmonth = (Time.zone.now + 330.minutes).month
        expyear = (Time.zone.now + 330.minutes).year
        cardtype = "Pay U"
      ...
      if pro_order_master.external_order_no.nil?
         ...
         orderline1 = OrderLine.where("orderid = ?", order_id).order("order_lines.id")
          ...
          if orderline2.present?
            ...
            #  puts "Adding product #{prod2} pieces #{qty2} to order l163"
          end
          orderline3 = OrderLine.where("orderid = ?", order_id).order("order_lines.id")
           .joins(:product_variant)
          .where.not('PRODUCT_VARIANTS.product_sell_type_id = ?', 10000)
          .where.not(product_list_id: add_on_replacements)
          .offset(1)
          ...
          if orderline3.present?
            ...
            #  puts "Adding product #{prod3} pieces #{qty3} to order l173"
          end
          ...
          if orderline4.present?
            ...
            #  puts "Adding product #{prod4} pieces #{qty4} to order l183"
          end
          ...
          if orderline5.present?
            ...
            #  puts "Adding product #{prod5} pieces #{qty5} to order l194"
          end
          ...
          if pro_order_master.external_order_no.blank?
            ...
            order_num = pro_order_master.external_order_no
          end
          ...
          if CustomerOrderList.where(ordernum: order_num).present?
           ...
           customer_order_list.update(order_id: order_id,
            ...
            title: (pro_order_master.customer.salute[0..4].upcase if pro_order_master.customer.salute.present?),
            ...
            city: pro_order_master.customer_address.city[0..19].upcase,
          ...
          pro_order_master.update(process_date: t)
          ...
          #add to sales ppo
          pro_order_master.delay(:queue => 'regular sales ppo', priority: 10).add_product_to_campaign_hbn_ppo #order_id
  ...
  def add_invoice_sms total_amount, alternative_mobile = nil
    ...
    encoded_url = URI.encode("amount=#{amount}&customerName=#{customerName}&description=#{description}&referenceId=#{transaction_ref}&customerMobileNumber=#{customerMobileNumber}&confirmSMSPhone=#{customerMobileNumber}")
    ...
    payumoney_detail = PayumoneyDetail.create(orderid: order_master.id,
      ...
      transaction_history: "Ref #{transaction_ref} created at #{this_time}",
      no_of_links: 1)
      
      #order_master.add_product_to_campaign_hbn_ppo
      order_master.delay(:queue => 'pay u to sales ppo', priority: 10).add_product_to_campaign_hbn_ppo #order_id
  ...
  def send_pay_u_sms_invoice total_amount, alternative_mobile = nil
    order_id = self.id
    this_time = Time.zone.now + 330.minutes
    current_time = Time.zone.now + 330.minutes
    order_master = OrderMaster.find(order_id)
  ...
  def regenerate_pay_u_sms_invoice order_id
    #order_id = self.id
    this_time = Time.zone.now + 330.minutes
    current_time = Time.zone.now + 330.minutes
    order_master = OrderMaster.find(order_id)
    ...
    amount = order_master.grand_total
    transaction_ref = SecureRandom.uuid
    customerName = "#{order_master.customer.salute.upcase} #{order_master.customer.first_name.upcase} #{order_master.customer.last_name.upcase}"
  ...
  def process_order order_id
      ...
      order_number = []
      cc_order_number = [] # Rs.<Variable1> towards ord no <Variable2>
      msg = nil
      order_total = 0
      ...
      if order_lines_regular.count > 1
          ...
          new_reg_order_lines.each do |order|
            ...
            order_number << " ord no #{order_split.external_order_no} for Rs.#{order_split.grand_total.to_i.to_s}"
            cc_order_number << " ord no #{order_split.external_order_no},"
          ...
          order_number << " ord no #{orderprocessed} for Rs.#{order_master.grand_total.to_i.to_s}"
          cc_order_number << " ord no #{orderprocessed},"


      else
         #IF ONLY one regular product found
      ...
      when 10001 #paid over COD
        message = "Thank you for your#{order_number.join(",")}. Your ord will reach you in 7-10 days. Please pay Cash on Delivery. For any queries call 9223100730 HBN/TELEBRANDS"
...
def financing_charges
  ...
  def add_product_to_campaign_hbn_ppo order_id = 0
  ...
  def updatemedia  order_id = 0
    ...
    puts "Checking for DNIS #{@order_master.calledno} ..."
      if @medialist.count == 1   #&& @all_calllist.empty?
          ...
          updates = "Updated at #{t} order for #{medianame} at Hour:#{nowhour}  Minutes:#{nowminute}"
          puts updates
              # update custorder with media
              custdetails = CUSTDETAILS.where(ordernum: external_order_no)
              if custdetails.present?
                custdetails.first.update(channel: @medialist.first.name)
                puts "Media updated in Oracle CCCRM with #{@medialist.first.name}"
              end



          @order_master.update(notes:  @order_master.notes + " " + updates)
        
          update_hbn_website_list order_id
        
          return   puts "Media #{medianame} added"
        end
           puts "#{@medialist.count} channels for same DNIS found, now checking for those from #{@order_master.customer_address.state}..."

           @medialist = @medialist.where("upper(state) = ?",@order_master.customer_address.state.upcase)
         ...
         #@newmedialist = Medium.where(active: 1).where('dnis = ? and state = ?', @order_master.calledno, @order_master.customer_address.state.upcase)
          ...
          updates = "Updated at #{t} order for #{medianame} for state #{@order_master.customer_address.state} at Hour:#{nowhour}  Minutes:#{nowminute}"
          puts updates
              #update custorder with media
              custdetails = CUSTDETAILS.where(ordernum: external_order_no)
               if custdetails.present?
                custdetails.first.update(channel: medianame)
                puts "Media updated in Oracle CCCRM with #{medianame}"
              end
          ...
          return puts " Media #{medianame} with state #{@order_master.customer_address.state.upcase} added "
        end
      
      
        #add_product_to_campaign(@newmedialist.first.id, order_id)
     return puts "More than one Channel details found for DNIS #{@order_master.calledno} do check if this is the HBN Channel"
