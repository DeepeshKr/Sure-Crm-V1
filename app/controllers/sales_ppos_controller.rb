class SalesPposController < ApplicationController
    ...
    def show_all
        ...
        elsif params.key?(:campaign_playlist_id)
    ...
    def details
        if params.key?(:campaign_id)
            ...
            @sales_ppos = SalesPpo.where('order_status_id > 10000')
                                  ...
                                  .order('start_time')
            if params.key?(:product_variant_id)
            ...
            main_product_type_id = 10_000
            basic_product_type_id = 10_040
            common_product_type_id = 10_001
    ...
    def half_hourly
        ...
        (@from_date.to_datetime.to_i..@to_date.to_datetime.to_i).step(30.minutes) do |date|
            ...
            media_cost_master = MediaCostMaster.where(media_id: 11_200).where('str_hr = ? AND str_min = ? AND end_hr = ? AND end_min = ?', start_hr, start_min, end_hr, end_min)
            ...
            ## Apply all the corrections here ###
            revenue *= @correction
            product_cost *= @correction
            product_damages = product_cost * 0.10
            totalorders *= @correction
    ...
    def product_performance
        ...
        return if @product_variant_id.nil? && @from_date.nil? && @to_date.nil?
        # @retail_default = 49
        @retail_default = SalesPpoDefault.find_by_name('Retail').value
        @retail_default = params[:retail_default] if params.key?(:retail_default)
        # @transfer_default = 65
        @transfer_default = SalesPpoDefault.find_by_name('Transfer Order').value
        if params.key?(:transfer_default)
    ...
    def show_performance
        ...
        @product_variants = ProductVariant.all.order('name').where('activeid = 10000')
        @total_media_cost = Medium.where(media_group_id: 10_000).sum(:daily_charges).to_f
        @hbn_media_cost = Medium.where(media_group_id: 10_000, active: true).sum(:daily_charges).to_f
    ...
    def half_hour_sales
        @hbn_media_cost_master = MediaCostMaster.where(media_id: 11_200).order('str_hr, str_min')
        @sno = 1
        @searchaction = 'hour_sales_performance'

        for_date = 330.minutes.from_now.to_date
        @from_date = 330.minutes.from_now.to_date
        @time_id = params[:time_id] if params.key?(:time_id)
        if params.key?(:from_date)
            @from_date = Date.strptime(params[:from_date], '%Y-%m-%d')
            @or_for_date = for_date.strftime('%Y-%m-%d')
        end
        ...
        return if @time_id.nil? || @from_date.nil? || @to_date.nil?
        ...
        @to_date.downto(@from_date).each do |day|
            ...
            orderlists.each do |order_list|
                ...
                next unless order_list.campaign_playlist
                products = ''
    ...
    def operator_sales_performance
        @sales_ppo_retail_default = SalesPpoDefault.find(10_000).value
        @sales_ppo_transfer_default = SalesPpoDefault.find(10_001).value

        @revised_per_day_rate = 0
        @medias = Medium.where(media_group_id: 10_000, active: true).where('id <> 11200').order(:name)
        ...
        if params.key?(:revised_per_day_rate)
        ...
        @from_date.upto(@to_date).each do |_day|
    ...
    def re_create_ppo
        ...
        if @campaign_playlist_id.present? || @product_variant_id.present?
            sales_ppos.each do |ppo|
                ...
                order_master.delay(queue: 'hbn sales ppo for product or campaign', priority: 100).add_product_to_campaign_hbn_ppo
                # ppo.delay.change_ppo_product_costs ppo.order_id
                @total_c_nos += 1
            end
            flash[:info] = "Your regeneration process would NOT include additional product variants or shows!"
        
        else
          # if no playlist id or product variant found then go by the dates only
          ...
          order_masters.each do |ord|
              ...
              ord.delay(queue: 'hbn sales ppos between dates', priority: 100).add_product_to_campaign_hbn_ppo # ord.id
              # ppo.order_id
              # create_sales_ppo ppo.order_id
              @total_c_nos += 1
              # byebug
          end
          flash[:info] = "Your regeneration process would INCLUDE additional product variants and all shows between dates!"
        end
            
        

        @in_time = (@total_c_nos / 60.00).round(2)
        @return_url = nil
        flash[:error] = "The process of recreating #{@total_c_nos} ppos has is started. The whole process is now handled by delayed job, which drips the data into Oracle in #{@in_time} mins"
        if params.key?(:return_url)
            @return_url = params[:return_url]
            redirect_to(@return_url) && return
        else
            redirect_to(sales_ppos_path) && return
        end
    end
    ...
    def re_calculate_order_id
      if params.key?(:order_id)
          @order_id = params[:order_id]
          #re_create_ppo = SalesPpo.new
          message = poco_add_product_to_campaign_hbn_ppo @order_id
          #message = re_create_ppo 
        if params.key?(:return_url)
          @return_url = params[:return_url]
          redirect_to @return_url, notice: "#{message}"
        else
          redirect_to sales_ppos_path, notice: "The process of recreating ppo for order id #{@order_id} is complete."
        end
      else
        redirect_to sales_ppos_path, notice: "The unable to re create ppo"
      end
    ...
    def hbn_fixed_costs
        fixed_pre_paid = [10_000, 10_045]
        @all_fixed_media = Medium.where(media_commision_id: fixed_pre_paid)
        @hbn_media = @all_fixed_media.where(media_group_id: 10_000, active: true, media_commision_id: fixed_pre_paid)
    ...
    def all_cancelled_orders
        @cancelled_status_id = [10_040, 10_006, 10_008]
    ...
    def daily_task_ppo_status
        ...
        @pay_u_orders =  OrderMaster.where('ORDER_STATUS_MASTER_ID = 10001')

        @pay_u_order_count = @pay_u_orders.distinct.count('id')
        @pay_u_order_value = @pay_u_orders.sum(:total)
        # upto
        @to_date.downto(@from_date).each do |day|
            ...
            hbnorders = OrderMaster.where('TRUNC(orderdate) = ?', day)
                                   .where('ORDER_STATUS_MASTER_ID > 10002').joins(:medium)
                                   .where('media.media_group_id = 10000').count
