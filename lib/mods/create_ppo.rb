module CreatePpo
  ...
  def poco_add_product_to_campaign_hbn_ppo order_id
    ...
    return puts "No product variant found in order" if product_variant_id.blank?
    
    puts "Checking for ordered product: #{product_variant.first.description} in order ref #{order_id}"
   ...
   if campaign_playlist.present?
     ...
     puts "Step 1: #{campaign_playlist_id} Order was for #{order_time.strftime("%d-%b-%Y")} #{nowhour}:#{nowminute} and Regular Campaign #{campaign_playlist.first.for_date.strftime("%d-%b-%Y")} #{campaign_playlist.first.start_hr}:#{campaign_playlist.first.start_min}" if campaign_playlist.present?
     ...
     puts "No recent campaign found for product variant id #{product_variant_id} when checked for #{nowsecs} sec for #{todaydate} => now will look for Extended campaign"
   ...
   if campaign_playlist_id.blank?
    ...
    if extended_campaign_playlist.present?
      puts "Step 2: Found extended campaign playlist #{campaign_playlist_id} in updated order #{order_id}" #..colorize(:white).colorize(:background => :red)
      campaign_playlist_id = extended_campaign_playlist.first.campaign_playlist_id if extended_campaign_playlist.present?
      
    else
      puts "No EXTENDED recent campaign found for product variant id #{product_variant_id} when checked for #{nowsecs} sec for #{todaydate} => now will look for older campaigns"
      
    end      
   end
   
    #check for additional product variants in extended file list end
    # cam = CampaignPlaylist.where(campaignid: campaignlist).where(productvariantid: product_variant_id).where("TRUNC(for_date) <= ?",  previous_date).where(list_status_id: 10000).order("for_date DESC, start_hr DESC, start_min DESC")
    # ##############
    #####################
   ...
   if !campaign_playlist_id.present?
     ...
     if older_campaign_playlist.present?
        ...
        campaign_playlist_id = older_campaign_playlist.first.id if older_campaign_playlist.first.id.present?
         ...
         puts "Step 3: #{campaign_playlist_id} Order was for #{order_time.strftime("%d-%b-%Y")} #{nowhour}:#{nowminute} and Older Campaign #{older_campaign_playlist.first.for_date.strftime("%d-%b-%Y")} #{older_campaign_playlist.first.start_hr}:#{older_campaign_playlist.first.start_min}" #..colorize(:red) if older_campaign_playlist.present?
   ...
   if campaign_playlist_id.blank?
     ...
     campaign_playlists.start_min DESC")
        ...
        if extended_older_campaign_playlist.present? && campaign_playlist_id.blank?
         puts "Step 4: Found campaign playlist #{campaign_playlist_id} in Extended Older Playlist and update this for order #{order_id}" #..colorize(:white).colorize(:background => :red)
    ...
    if campaign_playlist_id.present?
      puts "Post-check-Exit: Found campaign playlist #{campaign_playlist_id} in older campaign and updated this for order #{order_id}" #.colorize(:blue)
    else
      return puts "Exit: NO campaign playlist found in product campaign check" #.colorize(:red)
    end
  ...
  def create_sales_hbn_ppo order_id
    ...
    if order_master.order_status_master_id < 10001
      return puts "Not a valid order, skipped!".colorize(:blue)
    end
